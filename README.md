...
소스 코드 내의 취약점 탐지는 현대 사이버 보안에서 매우 중요한 과제로 여겨집니다.

본 프로젝트는 소스 코드의 패턴을 분석하여, 효율적으로 취약점을 탐지하는 것을 목표로 합니다. 

사전에 레이블링된 데이터(https://osf.io/d45bw/)를 활용해 C 언어 소스 코드를 학습하고, 취약 여부를 분류하며, 탐지된 취약점에 대한 수정 권장 사항을 제공합니다.

모델 구조와 가중치를 분리하는 접근법을 채택했습니다. 이는 실험 과정에서의 유연성과 모듈화를 강화하며, 가중치만 저장하는 방식은 저장 용량을 효율적으로 사용하고 모델 재사용성을 높입니다.
